...
- **주의/잔여**: (a) **참가자의 "진짜 룸 나가기"(members DELETE=본인 행 제거)는 여전히 미구현** — 참가자 나가기는 아직 로컬 refs만 제거(members 행 잔존). (b) **방장 삭제 시 다른 접속 참가자 화면**: 룸 삭제가 cells/members에 cascade DELETE를 일으켜 realtime DELETE 이벤트 발생 → 그들의 `fetchRoomState`가 방 없음으로 조용히 실패(에러 무시) → 화면엔 stale 보드가 남음(다음 열기 때 사라짐). 실시간 "삭제됨" 안내는 미구현. (c) **RLS 정책은 라이브 검증됨**: 임시 방(self-cleaning)으로 비방장 삭제=0(차단)·방장 삭제=1(허용) 확인, 기존 4개 방·실제 크로스유저 방(bd5a31c5) 무손상. **미검증은 UI 왕복만**(브라우저/실기기에서 방장에게 "삭제하기" 노출·클릭→deleteRoom·storage 정리·대시보드 복귀, 참가자에겐 "나가기" 노출).
