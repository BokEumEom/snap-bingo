...
### 이번 세션 (2026-07-15): **콘솔 제출 준비 — env 실기 진단 + 번들 경량화 + 초대 사전테스트 + 상세설명(README) + 스토어 스크린샷(세로·가로)**

- **env 실기기 미포함 진단**: 사용자가 "온디바이스에서 env가 안 들어간다" 보고. **원인** — `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY`는 `ait build`(→`vite build`) **빌드타임에 web 번들(`dist/web/assets/index-*.js`)로 인라인**되는 값(기기 런타임 주입 아님, `.env`는 기기에 없음). 콘솔에 올린 옛 `.ait`가 `.env` 생성 이전 빌드라 env가 안 박혀 함께(Supabase)가 `supabase=null`로 비활성. **해결** = `.env` 있는 상태로 재빌드→콘솔 재등록. **검증 함정** — `.ait`는 `AITBUNDL` 커스텀 포맷+web 자산 gzip 저장이라 raw `.ait` 평문 grep엔 안 잡히는 게 정상(오탐: `grep supabase.co`는 라이브러리 예시 URL에 걸림). **실제 프로젝트 ref 지문(`lwrkgxewyvwiumuqcoli`)으로 `dist/web/assets/index-*.js` 매칭 → 인라인 확인 완료**. (메모리 `snap-bingo-env-build`)
- **번들 경량화 (커밋 `2d637fd`)**: 미사용 에셋 21개 제거, **53M→19M(약 34MB↓)**. 슬림 재빌드 후 **deploymentId `019f5f75-e465-7c19-81a0-a72e73381ee2`** — 콘솔 등록 + **QR로 토스 실기 테스트 가능** 확인.
- **초대 실기 테스트 방법(출시 전)**: 프로덕션 스킴 `intoss://snap-bingo`는 미출시라 "출시 안 됨→서비스 이용 불가"로 안 열림. **사전 테스트는 피처 테스트 스킴** `intoss-private://appsintoss?_deploymentId=<ID>&queryParams=<URL-encoded JSON>`로 진입. 추천=파라미터 없음(`shareApp`), 함께=`?room=<uuid>`(`shareRoomInvite`). 인바운드는 프레임워크가 URL 정리 전 `index.html` 인라인 스크립트가 `window.__ENTRY_SEARCH`로 캡처 → `parseRoomId`/`parseInvite`.
- **상세 설명(스토어 카피) 함께 중심 재작성 → README 반영 (미커밋)**: 최상단 소개를 "친구들과 **함께 실시간으로 한 판을 채우는 빙고 챌린지**"로 교체. `주요 기능`의 옛 오기("원격 실시간 공동 편집 미지원"—구버전 잔재) 정정 → **함께(실시간 공동 빙고판, `shareRoomInvite`·`?room=`·`useSharedBoard`·RLS) + 같은 챌린지 각자 시작(솔로 복제 초대, `shareBoardInvite`)** 두 갈래로 정확히 서술.
- **스토어 스크린샷 생성 (`store-assets/`, 미추적 ~13MB, `.ait` 번들엔 미포함)**: 실행 앱을 실제 캡처(playwright + 시스템 chrome `channel:'chrome'` + Pretendard 설치) 후 마케팅 합성. **raw/**(390×844@2x) 4컷 + **marketing/**(세로 1080×1920) 4컷 + **marketing-landscape/**(가로 **1504×741**, AIT 콘솔 가로형) 4컷. 4컷 = 대시보드(함께 배지) / 함께 보드(초대 버튼+칸별 인증자 칩) / 인증 현황(BINGO 카드) / 여름 조각 갤러리. 시드: localStorage `photo_bingo_boards_v3`에 `shared:true`이되 id가 `room-`로 시작 안 하는 데모 보드(`seed_family`)→"함께" 배지 뜨되 Supabase 무접속(DB 무오염). 재생성 스크립트: 세로 `/tmp/snap-shot2.cjs`, 가로 `/tmp/snap-landscape.cjs`(raw 재활용, compose만). 다른 규격은 raw 재활용+캔버스 치수만 변경. (메모리 `snap-bingo-store-screenshots`)
- **현재 git 상태(핸드오프)**: `main` HEAD = `2d637fd`. **미커밋**: `README.md`(함께중심 재작성). **미추적**: `store-assets/`(~13MB, 세로+가로). 커밋 여부는 **사용자 결정 대기**(레포 +13MB지만 `.ait` 번들엔 영향 없음).
- **후속 계획**:
  - 🔴 **온디바 초대 실합류 검증**: 사전 테스트 스킴(`intoss-private://…&queryParams=…{room}`)으로 함께 방 딥링크 합류 성립 확인(초대 루프 핵심). 익명 세션(uid) 지속성도 함께.
  - **콘솔 제출 에셋**: 로고 600×600 업로드+발급 URL로 `granite.config.ts` `brand.icon` 교체, 스크린샷 업로드(가로형 `store-assets/marketing-landscape/` 사용). 콘솔이 별도 세로 규격 요구 시 raw 재활용해 재합성.
  - **env 재현성**: anon 키+URL은 공개 안전(RLS 보호, 이미 배포 번들에 포함) → OG 이미지 선례처럼 커밋 상수/`.env.production` 승격 검토(다른 PC·CI 빌드 시 조용한 누락 방지).

### 이번 세션 (2026-07-14): **전략 재정렬(리텐션) + 초대 모델 정리 + 진행도 라벨 + 익명 세션 지속성 + 함께 리텐션 루프 Phase 0**

- **전략 논의(코드 변경 전, 사용자와 압박테스트)**: 이 앱은 **계절 앱**이라 "지속성"은 *영원히*가 아니라 (a)이번 여름 붙잡기 (b)다음 시즌 복귀. **난이도 램프 배제** — 사진 빙고는 실력게임이 아니라 "어렵게"=마찰. 뱃지 6개(`lib/badges.ts`)는 **"보드 2개 완성"에서 flatline** — 나중에 가로 해금(새 테마/여름 마스터)으로 메움. **성장 엔진 = 초대/co-op**로 결론.
- **초대 모델 정리 (커밋 `060816a`)**: 홈=**앱 추천**(generic `shareApp`, DashboardView 헤딩·문구 "추천"으로 정리), 함께 보드=**룸 초대**(`shareRoomInvite`, 실시간 합류), **솔로 보드 초대 버튼 제거**(shared-only 게이팅). BoardDetailView 버튼 "이 챌린지에 친구 초대하기"로 통일. `shareBoardInvite`/수신 경로는 옛 링크 호환 위해 dormant 유지.
- **함께 카드 진행도 라벨 (커밋 `adbee21`)**: 대시보드 함께 카드가 '함께' 배지만 있던 걸 **솔로처럼 N/9·%**도 표시. 문제: `sharedRef.cells`는 갤러리용 '내 칸'만 담아 그대로 세면 항상 N/N·100% → **`BingoBoard.sharedProgress`(전체 9칸 중 누구든 채운 칸) 스냅샷 신설**(App이 룸 라이브 동기화 시 저장), DashboardView가 그걸로 계산. 갤러리 파이프라인(내 칸)은 불변. 트로피는 개인 성취라 솔로 전용 유지. 진행도는 **마지막 방 접속 시점 스냅샷**(라이브 아님).
- **익명 세션(uid) 지속성 강화 (커밋 `8b35c49`)**: **발견** — uid(=함께 보드 소유권, Supabase 익명 세션)가 supabase-js 기본값인 **웹뷰 localStorage**에 저장돼 토스 캐시삭제·eviction에 취약(정작 보드·닉네임은 튼튼한 토스 Storage였음). **수정**: `createClient auth.storage`에 토스 Storage 어댑터 주입(supabase-js v2 async storage, RN AsyncStorage와 동일 경로) → uid를 앱 단위 지속 Storage로 이동. `storage.ts`에 `removeStorageItem` 추가. 브라우저는 localStorage 폴백이라 동일. ⚠️ **실기 검증 필요**(토스 앱: 방 생성 후 재진입 시 같은 uid 유지).
- **함께 리텐션 루프 Phase 0 (서버 인프라 0, 익명 신원 그대로)**:
  - **share-back 넛지 (커밋 `b16889f`)**: 함께 칸 채운 직후 `MissionCompleteView`에 "친구들한테 알려주기"(shared-only) → fill 순간 문구로 `shareRoomInvite`(선택적 `message` 파라미터 신설). `completedCell`에 `shared` 플래그 추가. 솔로는 넛지 없음.
  - **'새 소식' 토스트 (커밋 `5a757a6`)**: 함께 방 재오픈 시 마지막 본 뒤 *남이* 새로 채운 칸 diff → "마지막 방문 후 OO님이 N칸 채웠어요". `lib/roomSeen.ts`(신규, 방별 last-seen `cell_index`, 토스 Storage), `useSharedBoard.news`(구독 세팅 **전에** 계산해 레이스 차단, 라이브로 본 칸도 last-seen 갱신해 재알림 방지). App이 토스트 1회(내 칸 제외).
- **리서치(Phase 1 준비, 앱인토스 문서 확인)**: **스마트 발송은 헤더 `x-anon-key`로 무로그인 발송 지원**(`x-user-key`=토스로그인 OR `x-anon-key`=익명). 단 우리 신원은 Supabase anon uid라 발송용 **앱인토스 `getAnonymousKey` 별도 발급·member 저장** 필요. 이벤트 알림은 `requestNotificationAgreement` 동의+콘솔 `templateSetCode` 템플릿, 10회/분/키, `POST /api-partner/v1/.../messenger/send-message`.
- **검증/배포**: 각 변경 `tsc --noEmit` 통과, 모두 `main` 푸시(…→`5a757a6`). 실제 공유시트·토스트·세션은 **토스 웹뷰에서만 실동작**(코드/타입만 검증). 메모리에 리텐션 로드맵 저장(`snap-bingo-retention-loop`).
- **후속 계획**:
  - 🔴 **실기 검증(온디바이스)**: 딥링크 `intoss://snap-bingo?room=...` 합류(초대 루프 성립의 핵심), 익명 세션 지속성.
  - **Phase 1 서버 푸시(제출 후)**: `getAnonymousKey`+동의+콘솔 템플릿+**Supabase Edge Function**(cell insert 감지→다른 멤버 anonKey 발송). 트리거=합류/진척digest/마일스톤(raw 칸마다 X=스팸), 안티스팸(쿨다운·quiet hours·앱에 있으면 스킵) 필수.
  - 뱃지 flatline 가로 해금(새 테마/여름 마스터 메타 뱃지).

### 이번 세션 (2026-07-14): **갤러리 → "여름 조각" 재편 (Phase 1: 9조각 카드 리스트 + 낱장 캐러셀)**

- **배경/결정 과정(사용자와 browse로 화면 확인 후 결정)**: "여름 9조각 콜라주 공유" 아이디어를 다듬다가, **인증현황(BingoAchievementView)의 souvenir 카드가 이미 '9조각 카드' 그 자체**임을 발견(3×3, 가운데 브랜드 워터마크, "나의 여름 조각들", 찍고빙고 브랜딩, 공유 버튼). 세 화면을 띄워 비교하니 **같은 사진이 3곳 중복** — 빙고판(기능·업로드 그리드) / 인증현황(공유용 카드) / 갤러리(낱장 2열 그리드). **갤러리 낱장 그리드가 제일 약함**("폰에 이미 있는데 왜?"). 사용자 결정(AskUserQuestion): **갤러리를 카드 리스트로 교체**, 탭 이름 **"여름 조각"**(앱이 이미 쓰는 "나의 여름 조각들" 카피와 일치). 흐름: **카드 리스트 → 탭 → 9장 낱장 캐러셀 + 공유**.
- **Phase 1 구현(구조)**:
  - **`PieceGrid`(신규)**: 3×3 souvenir 그리드 코어(가운데 워터마크, 칸=사진/빈점). 인증현황·갤러리·상세가 공유 → **중복 제거**.
  - **`SummerPieceCard`(신규)**: 전체 기념 카드(진행도 뱃지 + BINGO!/진행중 + PieceGrid + 푸터). `BingoAchievementView`가 인라인 카드를 이걸로 교체(**시각 회귀 0** — browse 확인).
  - **`SummerPieceDetail`(신규)**: 카드 탭 시 오버레이(`fixed inset-0 z-50`) = 큰 SummerPieceCard + **낱장 스크롤-스냅 캐러셀**(사진 있는 칸만, 슬라이드마다 제목·날짜·`completedBy` 크레딧) + "친구에게 공유하기". 카운터/도트는 scrollLeft로 인덱스 계산.
  - **`GalleryView` 재작성**: 낱장 그리드·필터칩 제거 → **인증 사진이 하나라도 있는 챌린지별 컴팩트 카드 리스트**(이모지+제목+N/N 뱃지+PieceGrid+"낱장으로 보기 ›"). 빈 상태·초기화(dev) 유지. `slide-clip-x`+`animate-slide-in-right` 유지.
  - **`BottomNav`**: 탭 라벨 `갤러리`→`여름 조각`.
- **공유(Phase 1)**: 기존 `shareApp`(텍스트+링크) 재사용. **콜라주 이미지 저장/공유 프리뷰는 미구현**(Phase 2/3).
- **검증(browse, 데모 보드는 localStorage에 hero 이미지로 시드 — DB 무오염)**: 인증현황 회귀 0 / 여름조각 탭 카드 리스트 정상 / 카드 탭→상세 큰 카드+캐러셀, 스크롤 시 카운터 1/9→3/9 갱신. **tsc 에러 0**.
- **후속 계획**:
  - **Phase 2 — 콜라주 저장**: `html-to-image`(이미 의존성, 미사용) → `saveBase64Data`로 앨범 저장. ⚠️ **과거 "#2 빙고 카드 이미지 저장"을 도입했다 롤백한 이력**(`saveCard.ts`·"이미지로 저장하기" 버튼 삭제)이 있으니 **재도입 전 왜 지웠는지 확인 필요**.
  - **Phase 3 — 공유 프리뷰(바이럴)**: 콜라주를 공개 https URL로 올려 `getTossShareLink(url, ogImageUrl)`에 태움 → 친구가 공유 카드에서 **내 콜라주**를 봄. 솔로 보드는 사진이 base64(localStorage)라 업로드 경로 신설 필요.

### 이번 세션 (2026-07-13): **스토리지 정책 강화 — 마이그레이션 008(남의 칸 사진 파일 덮어쓰기 차단)**

- **배경(사용자 확인)**: 직전 세션에서 남겨둔 선택 후속을 진행. 기존 `cell_photos_insert/update_member`는 경로의 **roomId(첫 세그먼트)만** 검사해 **방 멤버 누구나** `<roomId>/<cellIndex>.jpg`를 `upsert`로 덮어쓸 수 있었음. DB 행(`cells`)은 `cells_update_owner`로 보호되지만 **파일 바이트는 아니라**, 멤버가 남의 칸 사진을 훼손 가능(앱 UI로는 불가, 조작된 스토리지 API 호출로만). 사용자: "중요한 것 같다" → 적용.
- **마이그레이션 008 (커밋 `565d0da`, 라이브 적용)**: 파일 쓰기를 **그 칸 소유자**로 좁힘.
  - **`private.is_cell_owner(p_room uuid, p_cell_index int)`** SECURITY DEFINER 헬퍼 추가(`is_room_member`와 동일 패턴 — private 스키마·API 미노출, `cells.completed_by_uid = auth.uid()` 판정, `auth.uid()`는 호출자 JWT 유지).
  - `cell_photos_insert_member` → **`cell_photos_insert_owner`**, `cell_photos_update_member` → **`cell_photos_update_owner`**. 경로 파싱: roomId=`(storage.foldername(name))[1]::uuid`, cellIndex=`split_part(split_part(name,'/',2),'.',1)::int`.
  - **SELECT(`cell_photos_select_member`, 멤버 전체 열람)·DELETE(`cell_photos_delete_owner`, 방장/서버 Edge) 정책은 유지**. 클라이언트 코드 변경 없음(uploadThumb/changePhoto는 이미 '내 칸'만 씀).
  - **의존성**: 정책이 통과하려면 업로드 시점에 내 소유 `cells` 행이 있어야 함 → `claimCell`(소유 지정, `ignoreDuplicates`)이 `uploadThumb`보다 먼저 커밋되는 현재 흐름이 이를 보장.
- **라이브 검증(실 익명 JWT curl, 유저 A·B + 임시 방)**: A 자기 칸0 INSERT **200**, ① B가 A의 칸0 덮어쓰기(UPDATE) **RLS 403 차단**(`new row violates row-level security policy`), ② B 자기 칸1 인증+업로드 **200**, ③ B가 미소유 칸5 파일 생성(INSERT) **RLS 403 차단**, ④ A 자기 칸0 교체(UPDATE) **200**. 즉 **비소유 쓰기만 차단·정상 흐름 유지**. 테스트 방은 `delete-room` Edge fn으로 정리(Edge fn 재검증 겸함) → test rooms/members/cells/photos 모두 0, 라이브 방 `8a025c6a`(사진 4장)·total_rooms=1 온전, 프로젝트 orphan 0.
- **잔여**: 참고로 `cell-photos` 버킷은 `getPublicUrl`(공개 URL)이라 사진은 URL만 알면 읽힘(별개 이슈, 008 범위 아님). 남은 큰 항목은 여전히 **토스 로그인 연동(옛 방 소유 복구, anon uid 한계)**.
...
- **선택 후속(신규 문제 아님)**: 기존 스토리지 정책 `cell_photos_update_member`는 **방의 아무 멤버나** 남의 칸 사진 **파일**을 덮어쓸 수 있음(칸 소유 DB 행은 `cells_update_owner`로 보호, 앱 UI로는 불가). `changePhoto`는 내 칸만 건드리지만, 방어 강화하려면 이 정책을 "칸 소유자만"으로 조이는 마이그레이션 008이 가능. → **적용 완료**(위 "스토리지 정책 강화 — 마이그레이션 008" 세션 참고).
